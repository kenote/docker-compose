...
read_server_env() {
    ...
    if [[ $CONTAINER_ID == '' ]]; then
        sys_echo "${yellow}Portainer 宿主机未安装${plain}"
        return 1
    fi
    ...
    HTTP_PORT=`docker inspect $CONTAINER_ID | jq -r '.[0].NetworkSettings.Ports["8000/tcp"][0].HostPort'`
    HTTPS_PORT=`docker inspect $CONTAINER_ID | jq -r '.[0].NetworkSettings.Ports["9443/tcp"][0].HostPort'`
    if [[ $1 == '' ]]; then
        echo
...
sett_server_env() {
    ...
    if [[ $1 == 'save' ]]; then
        ...
        sys_echo " 配置 Portainer 宿主机"
        sys_echo "${green}-----------------------------${plain}"
        EXCLUDE_HTTP_PORT=$HTTP_PORT
        EXCLUDE_HTTPS_PORT=$HTTPS_PORT
    fi
    ...
    do
        ...
        if !(echo $_https_port | grep -E "$pattern_port" &> /dev/null); then
            sys_echo "${red}HTTPS端口格式错误！${plain}"
            continue
        fi
        lsof_port $_https_port $EXCLUDE_HTTPS_PORT
        if [[ $? == 1 ]]; then
            continue
        fi
        break
    ...
    if [[ $1 == 'save' ]]; then
        ...
        sed -i "s/$(cat .env | grep -E "^HTTPS_PORT=")/HTTPS_PORT=$_https_port/" .env
        docker-compose down
        docker-compose up -d
    fi
}
...
install_server() {
    sys_echo "${green}-----------------------------${plain}"
    sys_echo " 安装 Portainer 宿主机"
    sys_echo "${green}-----------------------------${plain}"

    while read -p "安装路径[portainer]: " _workdir
    do
        if [[ $_workdir = '' ]]; then
            _workdir="portainer"
        fi
    ...
    # 拉取 compose 及 配置文件
    wget --no-check-certificate -qO docker-compose.yml $REPOSITORY_RAW_COMPOSE/main/portainer/compose.yml
    wget --no-check-certificate -qO .env $REPOSITORY_RAW_COMPOSE/main/portainer/.env.example
...
show_menu() {
    ...
    max_num=8
    if [[ $num == '' ]]; then
        sys_echo "${green}Portainer 宿主机${plain}"
        echo
        sys_echo "${green} 0${plain}. 退出脚本"
        sys_echo "------------------------"
        sys_echo "${green} 1${plain}. 查看状态"
        sys_echo "${green} 2${plain}. 启动宿主机"
        sys_echo "${green} 3${plain}. 停止宿主机"
        sys_echo "${green} 4${plain}. 重启宿主机"
        sys_echo "------------------------"
        sys_echo "${green} 5${plain}. 安装宿主机"
        sys_echo "${green} 6${plain}. 卸载宿主机"
    ...
    2 | 3 | 4 )
        ...
        2 )
            if [[ $CONTAINER_STATUS == 'running' ]]; then
                confirm "Portainer 宿主机正在运行, 是否要重启?" "n"
                if [[ $? == 0 ]]; then
                    echo
        ...
        3 )
            if [[ $CONTAINER_STATUS == 'running' ]]; then
                ...
                echo
                sys_echo "${yellow}Portainer 宿主机已停止${plain}"
            fi
        ;;
        4 )
            echo
    ...
    5 ) # 安装宿主机
        ...
        read_server_env "only"
        if [[ $? == 0 ]]; then
            sys_echo "${yellow}Portainer 宿主机已经安装${plain}"
            echo
            read  -n1  -p "按任意键继续" key
            clear
            show_menu
        ...
        confirm "确定要安装 Portainer 宿主机吗?" "n"
        if [[ $? == 0 ]]; then
            clear
            install_server
            sys_echo "${green}Portainer 宿主机安装完毕${plain}"
            echo
            read  -n1  -p "按任意键继续" key
        fi
        clear
    ...
    6 ) # 卸载宿主机
        ...
        confirm "确定要卸载 Portainer 宿主机吗?" "n"
        if [[ $? == 0 ]]; then
            clear
            remove_server
            sys_echo "${green}Portainer 宿主机卸载完毕${plain}"
            echo
            read  -n1  -p "按任意键继续" key
        fi
        clear
    ...
    7 ) # 配置参数
        ...
        confirm "确定要重新配置 Portainer 宿主机吗?" "n"
        if [[ $? == 0 ]]; then
            clear
    ...
    8 ) # 升级容器
        ...
        confirm "确定要升级 Portainer 宿主机的容器吗?" "n"
        if [[ $? == 0 ]]; then
            clear
            sett_server_env "save"
            echo
